...
public class EarthQuakeClient2 {
    ...
    public void quakesWithFilter() {
        ...
        ArrayList<QuakeEntry> magFilteredList = filter(list, magFilter);
        ArrayList<QuakeEntry> finalList = filter(magFilteredList, depthFilter);

        for (QuakeEntry qe : finalList) {
            System.out.println(qe);
        }

        // Comment out the previous filtering code
        // Add code for the new filtering criteria

        Location Denver = new Location(39.7392, -104.9903);
        Filter distanceFilter = new DistanceFilter(Denver, 1000000.0);
        Filter phraseFilter = new PhraseFilter("end", "a");

        ArrayList<QuakeEntry> distanceFilteredList = filter(list, distanceFilter);
        ArrayList<QuakeEntry> finalList2 = filter(distanceFilteredList, phraseFilter);

        for (QuakeEntry qe : finalList2) {
            System.out.println(qe);
        }
    }
    ...
    public void testMatchAllFilter() {
        ...
        // Print the names of the filters used
